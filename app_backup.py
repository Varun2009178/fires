...
def load_sklearn_models(model_path):
    ...
    # Use pickle with encoding option for compatibility
    try:
        with open(model_path, "rb") as model_file:
            final_model = pickle.load(model_file, encoding='latin1')
    except Exception as e:
        # If that fails, try using joblib which handles numpy better
        import joblib
...
def run_app():
    ...
    if image:
        ...
        with st.spinner("Processing..."):
            ...
            confidence = probabilities[model_predict[0]] * 100  # Convert to percentage

            # Display results with confidence
            st.markdown("---")
            st.subheader("📊 Prediction Results")

            # Color-coded result based on prediction type
            ...
            if confidence < 70:
                st.warning(f"⚠️ Low confidence prediction ({confidence:.1f}%). Results may be uncertain. Consider reviewing the image quality or analyzing additional images from the same area.")

            # Show detailed probabilities
            with st.expander("📈 View Detailed Probabilities"):
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("No Wildfire Probability", f"{probabilities[0]:.1%}")
                with col2:
                    st.metric("Wildfire Probability", f"{probabilities[1]:.1%}")
